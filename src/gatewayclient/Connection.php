...
namespace axguowen\gatewayclient;

use axguowen\GatewayClient;
...
class Connection
{
     ...
     * @return $this
     */
    public function setClient(GatewayClient $client)
    {
        // 设置客户端
        $this->client = $client;
        // 返回自身
    ...
    protected function getAddresses()
    {
        ...
        $options = $this->config;
        // 通讯地址存在且缓存未过期
        if (!empty($this->addresses) && $requestTime - $this->updateTime < 1) {
            // 直接返回缓存的通讯地址
